// script.js - (with integrated validation + universal Paystack + navbar fix)
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  /* ======================
     Navbar active highlighting (universal)
     ====================== */
  function updateNavbarActive() {
    const url = location.href.toLowerCase();
    const mapping = [
      { match: "index.html", idx: 0 },
      { match: "select.html", idx: 1 },
      { match: "pickadons.html", idx: 2 },
      { match: "finishingup.html", idx: 3 },
      { match: "thankyou.html", idx: 3 }
    ];
    let activeIdx = 0;
    for (const p of mapping) { if (url.includes(p.match)) { activeIdx = p.idx; break; } }
  ...
  function initHandlersForRoot(root) {
    ...
    if (toggleInput) {
      ...
      if (toggleContainer) {
        ...
        toggleContainer.addEventListener("keydown",(e)=>{ if(e.key==="Enter"||e.key===" "){e.preventDefault();toggleInput.checked=!toggleInput.checked;toggleInput.dispatchEvent(new Event('change',{bubbles:true}));} });
      }
    }
    ...
    addonEls.forEach(a=>{
      ...
      if(!cb){ cb=document.createElement("input"); cb.type="checkbox"; a.prepend(cb);}
      cb.tabIndex=-1;
      a.setAttribute("tabindex","0");
      const savedAddons=load(STORAGE.ADDONS,[]);
      const isSel=savedAddons.find(x=>x.id===a.dataset.addon);
      cb.checked=!!isSel; a.classList.toggle("selected",!!isSel);
      const toggleAddon=()=>{ cb.checked=!cb.checked; a.classList.toggle("selected",cb.checked); persistAddonsFromRepresentativeRoot(); };
      a.addEventListener("click",(ev)=>{ if(ev.target.tagName.toLowerCase()!=="input") toggleAddon(); });
      a.addEventListener("keydown",(e)=>{ if(e.key==="Enter"||e.key===" "){e.preventDefault(); toggleAddon(); } });
      cb.addEventListener("change",()=>{ a.classList.toggle("selected",cb.checked); persistAddonsFromRepresentativeRoot(); });
    });
  }
  ...
  function persistAddonsFromRepresentativeRoot() {
    ...
    const rep=(desktop&&window.getComputedStyle(desktop).display!=="none")?desktop:(mobile&&window.getComputedStyle(mobile).display!=="none")?mobile:(desktop||mobile||document);
    const selected=[];
  ...
  (function attachIndexValidation(){
    ...
    getRoots().forEach(root=>{
      ...
      if(saved){ if(inputs[0]) inputs[0].value=saved.name||""; if(inputs[1]) inputs[1].value=saved.email||""; if(inputs[2]) inputs[2].value=saved.phone||""; }
      inputs.forEach(inp=>inp.addEventListener("input",()=>{ const data={name:inputs[0]?.value||"",email:inputs[1]?.value||"",phone:inputs[2]?.value||""}; save(STORAGE.PERSONAL,data); inp.classList.remove("invalid"); clearFormError(root); }));
      const nextAnchor=root.querySelector('.butt a[href="select.html"], .butt-2 a[href="select.html"]');
      if(nextAnchor) nextAnchor.addEventListener("click",(ev)=>{
        let ok=true;
        inputs.forEach(inp=>{ if(!inp.value||inp.value.trim()===""){ inp.classList.add("invalid"); ok=false;} else inp.classList.remove("invalid"); });
        const emailInput=inputs[1];
        if(emailInput&&emailInput.value&&!isValidEmail(emailInput.value)){ emailInput.classList.add("invalid"); ok=false; showFormError(root,"Please enter a valid email address."); }
        else if(ok) clearFormError(root);
        if(!ok){ ev.preventDefault(); const firstInvalid=root.querySelector(".invalid"); if(firstInvalid) firstInvalid.scrollIntoView({behavior:"smooth",block:"center"}); }
      });
    });
  })();
  ...
  (function attachSelectValidation(){
    ...
    getRoots().forEach(root=>{
      ...
      nextAnchor.addEventListener("click",(ev)=>{
        ...
        if(!hasSelection){ ev.preventDefault(); showFormError(root,"Please select a plan to continue."); const firstPlan=root.querySelector(".container[data-plan]"); if(firstPlan) firstPlan.scrollIntoView({behavior:"smooth",block:"center"});}
  ...
  (function renderFinishingUp(){
    ...
    let total=Number(plan.price)||0; addons.forEach(a=>total+=Number(a.price||0)); if(totalLabel) totalLabel.textContent=`Total (per ${billing==="monthly"?"month":"year"})`; if(totalSpan) totalSpan.textContent=`+$${total}/${billing==="monthly"?"mo":"yr"}`;

    function runPaystackFlow(ev){
      ...
      let computedTotal=Number(storedPlan&&storedPlan.price?storedPlan.price:plan.price)||0; storedAddons.forEach(a=>computedTotal+=Number(a.price||0));
      const amountKobo=Math.round(computedTotal*100);
      if(!personal||!personal.email||!isValidEmail(personal.email)){ showFormError(rep,"Please complete your email before payment."); return; } else clearFormError(rep);
      ...
      const handler=PaystackPop.setup({
        ...
        callback:function(response){ try{ save(STORAGE.BILLING,"monthly"); localStorage.removeItem(STORAGE.PLAN); localStorage.removeItem(STORAGE.ADDONS);}catch(e){console.warn("Storage clear error:",e);}
    ...
    if(confirmAnchor) confirmAnchor.addEventListener("click",runPaystackFlow);
    const payButton=rep.querySelector("#payButton");
    if(payButton) payButton.addEventListener("click",runPaystackFlow);

  })();

  /* ======================
     Window resize: re-sync
     ====================== */
